...
class Dijkstra<T : Any>(private val graph: Graph<T>) {
    ...
    private fun distance(destination: Vertex<T>, paths: MutableMap<Vertex<T>, Visit<T>>): Double = route(destination, paths).sumOf { it.weight ?: 0.0 }
    ...
    data class Visit<T : Any>(val type: VisitType, val edge: Edge<T>? = null)
