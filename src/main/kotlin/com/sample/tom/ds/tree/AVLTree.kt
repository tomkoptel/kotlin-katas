...
class AVLTree<T : Comparable<T>> {
        ...
        while (current != null) {
            if (value < current.value) {
                if (current.leftChild != null) {
                    ...
                    current.leftChild = AVLNode(value)
                    return
                }
            } else {
                if (current.rightChild != null) {
                    ...
                    current.rightChild = AVLNode(value)
                    return
                }
            }
        }
    ...
    private fun removeNode(node: AVLNode<T>, parent: AVLNode<T>?, isLeftChild: Boolean) {
        if (node.leftChild == null && node.rightChild == null) {
    ...
    private fun isBST(node: AVLNode<T>?, min: T?, max: T?): Boolean {
        node ?: return true
    ...
    val isBSTNonRecursive: Boolean
        get() {
            val stack = ArrayDeque<AVLNode<T>?>()
    ...
    override fun equals(other: Any?): Boolean {
        ...
        val otherTree = other as AVLTree<T>
        val currentTree = this

        val currentNodes = ArrayDeque<AVLNode<T>?>().also {
            it.addFirst(currentTree.root)
        }
        val otherNodes = ArrayDeque<AVLNode<T>?>().also {
    ...
    fun contains(tree: AVLTree<T>): Boolean {
